...
var mainScope = ( function (){
  ...
  document.getElementById('file').onchange = function handleFiles(input) {
    ...
    reader.onload = (event) => {
      ...
      var filtered = allLines.filter(function (el) {
      return el != null;
      });
      // Reading line and convert to array 
      inputArray =   allLines.map((line) => {
      console.log(Array.from(filtered));
      return Array.from(filtered);
      });
      alert(sudoku.init(inputArray).isValid());      
    };
